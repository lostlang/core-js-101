...
function findFirstSingleChar(str) {
  return str
    .split('')
    .find((c, i, arr) => arr.indexOf(c) === arr.lastIndexOf(c));
}
...
function isCreditCardNumber(ccn) {
  ...
  return (10 - (sum % 10)) % 10 === (`${ccn}`).split('').pop() - 0;
}
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(
    (`${num}`).split('').reduce((acc, cur) => acc + (cur - 0), 0),
  );
}
...
function getMatrixProduct(m1, m2) {
  ...
  for (let x = 0; x < m1.length; x += 1) {
    ...
    for (let y = 0; y < m2[0].length; y += 1) {
      out[x][y] = m2.reduce((a, e, i) => a + m1[x][i] * e[y], 0);
    }
  }
  return out;
}
...
function evaluateTicTacToePosition(position) {
  for (let x = 0; x < 3; x += 1) {
    if (
      position[x][0] && position[x][0] === position[x][1] && position[x][1] === position[x][2]
    ) {
      return position[x][0];
    }
    if (
      position[0][x] && position[0][x] === position[1][x] && position[1][x] === position[2][x]
    ) {
      return position[0][x];
    }
  }
